...
class Mission(BaseDocument):
    ...
    daily_rate = ListField(FloatField(min_value=0), max_length=2, description="Daily rate range for freelance contracts as a tuple (min, max)")
    annual_gross_salary = ListField(FloatField(min_value=1), max_length=2, description="Annual gross salary range as a tuple of minimum and maximum")
